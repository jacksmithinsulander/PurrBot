...
mod tests {
    ...
    async fn test_ethereum_key_generation_and_recovery() {
        ...
        // Get the private key from the in-memory wallet
        let original_private_key = handler.get_private_key().await.unwrap().unwrap();
        let original_public_key = handler.get_public_key().await.unwrap().unwrap();
        
        // Create a new handler (simulating a new session)
        let handler2 = PasswordHandler::new(config_store).unwrap();
        
        // Login with the same credentials
        let login_success = handler2.login(user_id, password).await.unwrap();
        assert!(login_success);
        
        // Get the keys after login
        let recovered_private_key = handler2.get_private_key().await.unwrap().unwrap();
        let recovered_public_key = handler2.get_public_key().await.unwrap().unwrap();
        
        // Verify the keys match
        assert_eq!(original_private_key, recovered_private_key);
        assert_eq!(original_public_key, recovered_public_key);
        
        // Test wrong password
        let handler3 = PasswordHandler::new(Arc::new(UserConfigStore::new(db_path).unwrap())).unwrap();
        let login_fail = handler3.login(user_id, "wrong_password").await.unwrap();
        assert!(!login_fail);
    }
}
