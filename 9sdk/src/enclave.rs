...
pub struct EnclaveManager {
    config: Mutex<Option<EnclaveConfig>>,
...
impl EnclaveManager {
    ...
    fn handle_request(&self, request: EnclaveRequest) -> Result<EnclaveResponse, EnclaveError> {
        match request {
    ...
    pub fn setup_config(&self, password: &str) -> Result<String, EnclaveError> {
    ...
    pub fn verify_and_derive_keys(
        ...
        let cfg = config_guard.as_ref().ok_or(EnclaveError::InvalidConfig)?;
...
mod tests {
    ...
    fn test_password_verification() {
        let manager = EnclaveManager::new().unwrap();
        let password = "test_password";

        // Setup config
        manager.setup_config(password).unwrap();

        // Verify correct password
        let (key1, key2) = manager.verify_and_derive_keys(password).unwrap();
        assert_eq!(key1.len(), 32);
        assert_eq!(key2.len(), 32);

        // Verify incorrect password
        assert!(matches!(
            manager.verify_and_derive_keys("wrong_password"),
